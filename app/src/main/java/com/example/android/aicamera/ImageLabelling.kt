...
class ImageLabelling : Fragment() {
    ...
    private val viewModel: ImageLabellingViewModel by lazy {
        ViewModelProvider(this, ImageLabellingViewModel.Factory(activity!!.application))[ImageLabellingViewModel::class.java]
    }
            ...
            when (item.itemId) {
                R.id.action_camera_flip -> {
                    ...
                    if(viewModel.which_camera == 0) {
                        binding.objToolbar.menu.findItem(R.id.action_flash).isVisible = false
                        viewModel.isFlash = false
                    }
                    else {
                        binding.objToolbar.menu.findItem(R.id.action_flash).isVisible = true
